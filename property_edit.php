...
// 1. Soo qaado xogta hadda jirta (Fetch)
if (isset($_GET['id'])) {
    $id = mysqli_real_escape_string($con, $_GET['id']);
    $query = "SELECT pb.*, p.title as property_name FROM property_bookings pb 
...
// Hubi haddii status-ka oo kaliya uu yahay mid hore loo xiray
$is_status_locked = ($row['status'] == 'Completed' || $row['status'] == 'Canceled');

// 2. Marka la riixo Update (Process Form)
if (isset($_POST['update_booking'])) {
    $id = mysqli_real_escape_string($con, $_POST['booking_id']);
    ...
    // PHP Logic: Haddii status-ku xirnaa, ha oggolaan in la beddelo status-ka
    // Laakiin xogta kale waad beddeli kartaa
    if ($is_status_locked) {
        $status = $row['status']; // Isticmaal kii hore ee xirnaa
    } else {
        $status = mysqli_real_escape_string($con, $_POST['status']);
    }

    $sql = "UPDATE property_bookings SET 
            full_name = '$full_name', 
            phone = '$phone', 
            email = '$email',
            city = '$city', 
    ...
    if (mysqli_query($con, $sql)) {
        $success = "Xogta macmiilka waa la cusubaysiiyay!";
        header("Refresh: 2; url=property_list.php");
    } else {
        $error = "Cilad: " . mysqli_error($con);
    }
}
?><!DOCTYPE html>
    ...
    <div class="nxl-content">
        <div class="page-header">
            <div class="page-header-left">
                <h5 class="m-b-10">Edit Property Inquiry</h5>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="property_requests.php">Inquiries</a></li>
                ...
                <div class="col-lg-8 mx-auto">
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-bold">Update Customer Information</h6>
                            <?php if ($is_status_locked): ?>
                            <span class="badge bg-soft-danger text-danger border border-danger-subtle">
                                <i class="feather-lock me-1"></i> Status Locked
                            </span>
                            <?php endif; ?>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="alert alert-danger alert-dismissible fade show">
                                <i class="feather-alert-octagon me-2"></i> <?php echo $error; ?>
                            </div>
                            <?php endif; ?>
                            ...
                            <form action="" method="POST">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label">Nationality (Origin)</label>
                                        <input type="text" name="nationality" class="form-control"
                                            value="<?php echo htmlspecialchars($row['nationality']); ?>">
                                    </div>

                                    <div class="col-md-12">
                                        <label class="form-label fw-bold <?php echo $is_status_locked ? 'text-danger' : 'text-primary'; ?>">
                                            Inquiry Status <?php echo $is_status_locked ? '(Locked)' : ''; ?>
                                        </label>
                                        <select name="status" class="form-select <?php echo $is_status_locked ? 'bg-light text-muted' : 'border-primary'; ?>"
                                            <?php echo $is_status_locked ? 'disabled' : ''; ?>>
                                        ...
                                        <?php if ($is_status_locked): ?>
                                            ...
                                            <small class="text-muted mt-1 d-block italic">Note: Status-ka lama beddeli karo ka dib marka la xiro.</small>
                                        <?php endif; ?>
                                    </div>

                                    <div class="col-md-12">
                                        <label class="form-label fw-bold text-muted">Property (Read Only)</label>
                                        <input type="text" class="form-control bg-light border-dashed"
                                            value="<?php echo htmlspecialchars($row['property_name'] ?? 'General Inquiry'); ?>"
                                            readonly>
                                    </div>
                                </div>

                                <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                                    <a href="property_list.php" class="btn btn-light-danger px-4">
                                        <i class="feather-arrow-left me-1"></i> Back
                                    </a>
                                    <button type="submit" name="update_booking" class="btn btn-primary px-4">
                                        <i class="feather-check-circle me-1"></i> Save Changes
                                    </button>
                                </div>
                            </form>
                        </div>
